...
def get_shakespeare_text(limit=10000):
    file_path = Path("Datasets/tinyshakespeare.txt")
    ...
    return text[:limit]  # Limit text to specified number of characters
...
def train():
    ...
    # Set dataset choice from config
    dataset_choice = config.dataset  # "shakespeare", "wikitext", or "both"

    # Get text data based on the dataset choice
    if dataset_choice == "shakespeare" or dataset_choice == "both":
        shakespeare_text = get_shakespeare_text(limit=config.shakespeare_limit)
        shakespeare_tokenizer = CharacterTokenizer(shakespeare_text)

    if dataset_choice == "wikitext" or dataset_choice == "both":
        wikitext_data = get_wikitext_data(limit=config.wikitext_limit)
        wikitext_tokenizer = CharacterTokenizer(wikitext_data)

    # Use the appropriate tokenizer based on the primary dataset
    if dataset_choice == "wikitext":
        primary_tokenizer = wikitext_tokenizer
        primary_text = wikitext_data
    else:
        primary_tokenizer = shakespeare_tokenizer
    ...
    # Create evaluation datasets if using both
    if dataset_choice == "both":
        # Use a small subset for evaluation
        shakespeare_eval_dataset = TextDataset(
            shakespeare_text[-min(len(shakespeare_text) // 5, 10000) :],
            seq_length,
            shakespeare_tokenizer,
        )
        shakespeare_eval_dataloader = DataLoader(
            shakespeare_eval_dataset, batch_size=config.batch_size, shuffle=False
        )

        wikitext_eval_dataset = TextDataset(
            wikitext_data[-min(len(wikitext_data) // 5, 10000) :],
            seq_length,
            wikitext_tokenizer,
        )
        wikitext_eval_dataloader = DataLoader(
            wikitext_eval_dataset, batch_size=config.batch_size, shuffle=False
        )
    ...
    for epoch in range(config.epochs):
        ...
        # Evaluate on both datasets if using both
        if (
            dataset_choice == "both" and epoch % 5 == 0
        ):  # Evaluate every 5 epochs to save time
            shakespeare_perplexity = evaluate_perplexity(
                model, shakespeare_eval_dataloader, criterion, device
            )
            wikitext_perplexity = evaluate_perplexity(
                model, wikitext_eval_dataloader, criterion, device
            )

            metrics.update(
                {
                    "shakespeare_perplexity": shakespeare_perplexity,
                    "wikitext_perplexity": wikitext_perplexity,
                }
            )

            print(f"\nEpoch {epoch}:")
            print(f"  Shakespeare Perplexity: {shakespeare_perplexity:.2f}")
            print(f"  WikiText Perplexity: {wikitext_perplexity:.2f}")
...
if __name__ == "__main__":
    ...
    experiments = [
        ...
        {
            # Experiment 2: Adam with GELU
            "optimizer": "adam",
            "activation": "gelu",
        },
        {
            # Experiment 3: AdamW with GELU
            "optimizer": "adamw",
            "activation": "gelu",
        },
    ...
    final_losses = {depth: {"relu": None, "gelu": None} for depth in depths}
    ...
    for exp in experiments:
        ...
        with wandb.init(project=project_name, config=config):
            ...
            final_losses[config["num_layers"]][config["activation"]] = final_loss

    # Prepare data for CSV
    csv_data = [["Depth", "Final Loss with GELU", "Final Loss with ReLU"]]
    for depth in depths:
        gelu_loss = final_losses[depth]["gelu"]
        relu_loss = final_losses[depth]["relu"]
        csv_data.append([depth, gelu_loss, relu_loss])
