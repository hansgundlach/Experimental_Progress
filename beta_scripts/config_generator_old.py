...
def find_optimal_architecture(target_params, activation, vocab_size=50257):
    ...
    # For very small models, we need to reduce vocab_size or use a different approach
    # Let's use a smaller search space and be more aggressive about small models

    # Adjust search range based on target parameters
    if target_params < 1e6:  # Less than 1M params
        min_hidden = 16  # Start smaller
        max_hidden = min(128, int(math.sqrt(target_params / 1000)))  # Much smaller max
    else:
        min_hidden = 32
        max_hidden = int(4096 * (target_params / 1e8) ** 0.5)

    # Ensure max_hidden is at least min_hidden
    max_hidden = max(max_hidden, min_hidden)

    print(
        f"  Searching architectures for {target_params:,} params: hidden_dim range [{min_hidden}, {max_hidden}]"
    )

    # Iterate through reasonable hidden dimensions (must be divisible by 16)
    for hidden_dim in range(min_hidden, max_hidden + 1, 16):
        # For small models, allow smaller head dimensions
        if target_params < 1e6:
            head_dims = [16, 32, 64]  # Allow smaller heads for tiny models
        else:
            head_dims = [64, 128]  # Standard head dimensions
        ...
        for head_dim in head_dims:
            ...
            # Skip if num_heads is 0
            if num_heads == 0:
                continue

            # Calculate parameter components
            embedding_params = vocab_size * hidden_dim
            output_params = hidden_dim * vocab_size
            ...
            # Solve for L (number of layers)
            required_layers = (target_params - non_layer_params) / layer_params
            num_layers = max(1, round(required_layers))

            # For very small targets, allow 0 layers (just embedding + output)
            if target_params < non_layer_params and target_params < 1e6:
                num_layers = 1  # Minimum viable transformer

            params = non_layer_params + num_layers * layer_params
            diff = abs(params - target_params)

            # Debug output for first few iterations
            if len(str(best_diff)) > 10:  # Only print for first iteration
                print(
                    f"    Testing {hidden_dim}d x {num_layers}L x {num_heads}H = {params:,} params (diff: {diff:,})"
                )
    ...
    if best_config is None:
        # Fallback: create a minimal viable architecture
        print(
            f"  Warning: Could not find optimal architecture for {target_params:,}, using minimal config"
        )
        return (16, 1, 1)  # Minimal transformer

    hidden_dim, num_layers, num_heads = best_config
    actual_params = vocab_size * hidden_dim * 2 + num_layers * calculate_layer_params(
        hidden_dim, activation
    )
    print(
        f"  Selected: {hidden_dim}d x {num_layers}L x {num_heads}H = {actual_params:,} params (target: {target_params:,})"
    )
