...
from data_loading import get_dataset_smart, TextDataset
...
class SwiGLU(nn.Module):
    def __init__(self, dim, hidden_dim=None, dropout=0.0, use_bias=True):
        super().__init__()
        ...
        self.to_out = nn.Linear(hidden_dim, dim, bias=use_bias)
...
class GLUFeedForward(nn.Module):
    def __init__(self, dim, dropout=0.1, config=None):
        super().__init__()
        # Get ff_ratio from config, default to 4
        ff_ratio = getattr(config, "ff_ratio", 4) if config else 4
        modern_bias_0 = getattr(config, "modern_bias_0", False) if config else False
        use_bias = not modern_bias_0
        # Calculate dimensions and ensure they are integers
...
class SimpleTransformerLayer(nn.Module):
    def __init__(self, hidden_dim, num_heads, dropout=0.1, config=None):
        ...
        if norm_type == "rms":
            ...
            if modern_bias_0:
                # LayerNorm without bias (only scale parameter)
                norm_layer = lambda dim: nn.LayerNorm(dim, bias=False)
            else:
        ...
        # Determine if we should use bias based on modern_bias_0
        use_bias = not modern_bias_0  # No bias if modern_bias_0 is True

        self.qkv = Linear(hidden_dim, 3 * hidden_dim, bias=use_bias)
        self.out_proj = Linear(hidden_dim, hidden_dim, bias=use_bias)

        if config.activation == "swiglu":
            # SwiGLU is a complete feed-forward module
            ff_ratio = getattr(config, "ff_ratio", 4)
            ff_dim = int(hidden_dim * ff_ratio)  # Ensure integer dimension
            self.ff = SwiGLU(hidden_dim, ff_dim, dropout=dropout, use_bias=use_bias)
        elif config.activation == "glu":
            self.ff = GLUFeedForward(hidden_dim, dropout=dropout, config=config)
        else:
...
class SimpleTransformer(nn.Module):
    def __init__(
        ...
        # Determine if output layer should have bias
        modern_bias_0 = getattr(config, "modern_bias_0", False)
        ...
        self.fc = Linear(hidden_dim, vocab_size, bias=use_output_bias)
...
def train(gpu_id=None, csv_log_path=None):
    ...
    train_dataset, val_dataset, primary_tokenizer, primary_text = get_dataset_smart(
        config
    )
...
# get_dataset function moved to data_loading.py
